...
// const MOCK_DATA = {};
// const MOCK_TYCOONS = ['tycoon-id-1', 'tycoon-id-2', 'tycoon-id-3', 'tycoon-id-4', 'tycoon-id-5'];
// const TYCOON_ID_CORP_ID_COMPANY_IDS = {
//   'tycoon-id-1': {
...
export default class MockBuildingFactory {
  ...
  // setupMocks () {
  //   const mock_map_buildings = [];
  //   const can_place = (xt, yt, info) => {
  //     for (let j = 0; j < info.h; j++) {
  //       for (let i = 0; i < info.w; i++) {
  //         const index = 1000 * (yt - j) + (xt - i);
  //         if (mock_map_buildings[index] || RoadManager.DUMMY_ROAD_DATA[index] || this.client_state.planet.game_map?.ground_map?.is_coast_at(xt - i, yt - j) || this.client_state.planet.game_map?.ground_map?.is_water_at(xt - i, yt - j) && @client_state.planet.game_map?.ground_map?.is_coast_around(xt - i, yt - j)) return false;
  //       }
  //     }
  //     return true;
  //   };
